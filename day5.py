...
async def process_l5_step_9(callback_query, state):
    await state.set_state(LessonStates5.step_10)
    await callback_query.message.answer(
        "Любой результат — норма, ведь пищевые привычки сложно менять. Но вместе у нас получится. Читай текст, в котором мы с нутрициологом объясняем, почему подобные привычки мешают нам питаться осознанно и советуем, как от них избавиться 👇",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
